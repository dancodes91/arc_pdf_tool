...
class DiffEngineV2:
    ...
    def create_diff(self, old_book: Dict[str, Any], new_book: Dict[str, Any]) -> DiffResult:
        ...
        """
        self.logger.info(
            f"Creating diff: {old_book.get('id', 'unknown')} -> {new_book.get('id', 'unknown')}"
        )
        ...
        review_queue = [match for match in matches if match.confidence < self.review_threshold]
        ...
        self.logger.info(
            f"Diff complete: {len(changes)} changes, {len(review_queue)} items need review"
        )
        return result
    ...
    def _extract_products(self, book_data: Dict[str, Any]) -> List[Dict[str, Any]]:
        ...
        raw_products = (
            book_data.get("products", [])
            or book_data.get("items", [])
            or book_data.get("product_data", [])
            or []
        )
    ...
    def _match_products(
        self, old_products: List[Dict], new_products: List[Dict]
    ) -> List[MatchResult]:
        ...
        matched_old_keys = {
            match.old_item["_match_key"] for match in exact_matches if match.new_item
        }
        matched_new_keys = {
            match.new_item["_match_key"] for match in exact_matches if match.new_item
        }
        ...
        if self.enable_fuzzy_matching and unmatched_old and unmatched_new:
            ...
            fuzzy_old_keys = {
                match.old_item["_match_key"] for match in fuzzy_matches if match.new_item
            }
            fuzzy_new_keys = {
                match.new_item["_match_key"] for match in fuzzy_matches if match.new_item
            }
    ...
    def _fuzzy_matching(
        ...
        for old_product in old_products:
            ...
            fuzzy_results = process.extract(
                old_search_text, new_search_texts, scorer=fuzz.ratio, limit=3
            )
    ...
    def _validate_fuzzy_match(self, old_product: Dict, new_product: Dict) -> bool:
        ...
        if old_model and new_model:
            ...
            if (
                min_length > 0 and len(common_chars) < min_length * 0.2
            ):  # At least 20% common characters
                return False
    ...
    def _detect_changes(
        self, matches: List[MatchResult], old_book: Dict, new_book: Dict
    ) -> List[Change]:
    ...
    def _detect_item_changes(self, match: MatchResult) -> List[Change]:
        ...
        if old_price != new_price:
            ...
            if old_price and old_price > 0:
                ...
                description = (
                    f"Price changed {percent_change:+.1f}%: ${old_price:.2f} → ${new_price:.2f}"
                )
            else:
    ...
    def _detect_option_changes(self, old_book: Dict, new_book: Dict) -> List[Change]:
        ...
        old_option_lookup = {
            option.get("option_code", option.get("code")): option for option in old_options
        }
        new_option_lookup = {
            option.get("option_code", option.get("code")): option for option in new_options
        }
    ...
    def _extract_rules(self, book: Dict) -> List[Dict]:
        ...
        raw_rules = book.get("price_rules", []) or book.get("rules", []) or []
    ...
    def _extract_options(self, book: Dict) -> List[Dict]:
        ...
        raw_options = (
            book.get("hinge_additions", [])
            or book.get("options", [])
            or book.get("additions", [])
            or []
        )
    ...
    def _generate_summary(
        self, matches: List[MatchResult], changes: List[Change]
    ) -> Dict[str, int]:
        """Generate summary statistics."""
        summary = {
            "total_matches": len(matches),
            "exact_matches": len(
                [m for m in matches if m.confidence_level == MatchConfidence.EXACT]
            ),
            ...
            "option_changes": len(
                [c for c in changes if c.change_type.value.startswith("option_")]
            ),
