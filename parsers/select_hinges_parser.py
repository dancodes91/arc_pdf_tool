...
class SelectHingesParser(BasePDFParser):
    ...
    def _is_product_table(self, table: pd.DataFrame) -> bool:
        ...
        product_indicators = [
            'sku', 'model', 'price', 'hinge', 'door', 'hardware',
            'select', 'series', 'size', 'finish'
        ]
    ...
    def _extract_products_from_table(self, table: pd.DataFrame) -> List[Dict[str, Any]]:
        ...
        for idx, row in table.iterrows():
            try:
                ...
                size = str(row.iloc[size_col]) if size_col is not None and size_col < len(row) else ""
    ...
    def _find_sku_column(self, table: pd.DataFrame) -> Optional[int]:
        ...
        for col_idx, col in enumerate(table.columns):
            ...
            if any(indicator in col_text for indicator in ['sku', 'part', 'number', 'item', 'model']):
                return col_idx
    ...
    def _find_description_column(self, table: pd.DataFrame) -> Optional[int]:
        ...
        for col_idx, col in enumerate(table.columns):
            col_text = str(col).lower()
            if any(indicator in col_text for indicator in ['description', 'desc', 'name', 'type', 'series']):
                return col_idx
    ...
    def _find_size_column(self, table: pd.DataFrame) -> Optional[int]:
        ...
        for col_idx, col in enumerate(table.columns):
            col_text = str(col).lower()
            if any(indicator in col_text for indicator in ['size', 'dimension', 'width', 'height', 'length']):
                return col_idx
