...
class HagerParser:
    ...
    def _parse_price_rules(self, text: str) -> None:
        ...
        for page in self.document.pages[:max_pages_to_check]:
            ...
            if not any(
                keyword in page_upper
                for keyword in ["FINISH PRICE", "USE PRICE OF", "PRICE MAPPING"]
            ):
                continue
    ...
    def _parse_hinge_additions(self, text: str) -> None:
        ...
        for page in self.document.pages[:max_pages_to_check]:
            ...
            if not any(
                keyword in page_upper for keyword in ["ADDITION", "OPTION", "EPT", "ETW", "EMS"]
            ):
                continue
    ...
    def _parse_item_tables(self, text: str, tables: List[Any]) -> None:
        ...
        product_keywords = [
            ...
            "US4",
            "US10",
            "US26",  # Finish codes indicate product pages
            "Size",
            "Heavy",
            "Standard",
        ]
    ...
    def _validate_results(self, results: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if total_items > 0:
            ...
            for item_list in [
                results["products"],
                results["finish_symbols"],
                results["price_rules"],
                results["hinge_additions"],
            ]:
