...
def create_minimal_test_data():
    ...
    return {
        ...
        'options': [
            {'manufacturer': 'Hager', 'code': 'EPT', 'label': 'Electric Power Transfer',
             'add_type': 'net_add', 'amount': 41.00, 'notes': '', 'constraints_json': '{}', 'page_ref': 'p45'},
            {'manufacturer': 'Hager', 'code': 'CTW-4', 'label': 'Continuous Weld 4 inch',
             'add_type': 'net_add', 'amount': 108.00, 'notes': '', 'constraints_json': '{}', 'page_ref': 'p67'},
            {'manufacturer': 'Hager', 'code': 'EMS', 'label': 'Electrified Mortise',
             'add_type': 'net_add', 'amount': 65.00, 'notes': '', 'constraints_json': '{}', 'page_ref': 'p89'},
        ],
        'items': [
            {'manufacturer': 'Hager', 'family': 'Hinges', 'model_code': 'BB1191',
             'description': '4-1/2" x 4-1/2" Ball Bearing Hinge', 'finish': 'US3',
             'size_attr': '4.5x4.5', 'base_price': 45.50, 'currency': 'USD', 'page_ref': 'p123'},
            {'manufacturer': 'Hager', 'family': 'Hinges', 'model_code': 'BB1191',
             'description': '4-1/2" x 4-1/2" Ball Bearing Hinge', 'finish': 'US10B',
             'size_attr': '4.5x4.5', 'base_price': 48.75, 'currency': 'USD', 'page_ref': 'p123'},
        ],
        'rules': [
            {'type': 'inherit_price', 'source_finish': 'US10B', 'target_finish': 'US10A',
...
def test_csv_export(data, output_dir):
    ...
    with open(items_path, 'w', newline='', encoding='utf-8') as f:
        fieldnames = ['manufacturer', 'family', 'model_code', 'description', 'finish',
                     'size_attr', 'base_price', 'currency', 'page_ref']
    ...
    with open(options_path, 'w', newline='', encoding='utf-8') as f:
        fieldnames = ['manufacturer', 'code', 'label', 'add_type', 'amount', 'notes',
                     'constraints_json', 'page_ref']
        writer = csv.DictWriter(f, fieldnames=fieldnames)
        writer.writeheader()
...
def main():
    ...
    if items_path.exists():
        with open(items_path, 'r') as f:
            ...
            expected_headers = ['manufacturer', 'family', 'model_code', 'description', 'finish',
                              'size_attr', 'base_price', 'currency', 'page_ref']
    ...
    if summary_path.exists():
        ...
        with open(summary_path, 'r') as f:
            summary = json.load(f)
            if (summary.get('HasEffectiveDate') is True and
                summary.get('EffectiveDate') == '2025-03-31'):
