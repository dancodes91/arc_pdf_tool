...
def apply_renames(
    products: List[Dict[str, Any]], pct: float
...
def run_synthetic_test(
    rename_pct: float, required_accuracy: float = 0.95, product_count: int = 100
) -> bool:
    """
    ...
    diff_engine = DiffEngineV2(
        {"enable_fuzzy_matching": True, "fuzzy_threshold": 70, "review_threshold": 0.6}
    )
    ...
    if passed:
        logger.info(
            f"✅ TEST PASSED - Accuracy {accuracy * 100:.1f}% >= {required_accuracy * 100:.1f}%"
        )
    else:
        logger.info(
            f"❌ TEST FAILED - Accuracy {accuracy * 100:.1f}% < {required_accuracy * 100:.1f}%"
        )
    ...
    if fuzzy_matches:
        ...
        for match in fuzzy_matches[:5]:
            ...
            logger.info(
                f"  {old_model:20s} -> {new_model:20s} ({confidence * 100:.1f}% confidence)"
            )
...
def main():
    ...
    parser.add_argument(
        "--pct", type=float, default=0.2, help="Percentage to rename (0.0-1.0, default: 0.2)"
    )
    parser.add_argument(
        "--threshold", type=float, default=0.95, help="Required accuracy (0.0-1.0, default: 0.95)"
    )
    ...
    success = run_synthetic_test(
        rename_pct=args.pct, required_accuracy=args.threshold, product_count=args.count
    )
