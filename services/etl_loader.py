...
from database.models import (
    Base, Manufacturer, PriceBook, ProductFamily, Product,
    Finish, ProductPrice, ProductOption, ChangeLog
)
from parsers.shared.provenance import ParsedItem
...
class ETLLoader:
    ...
    def _load_price_book(self, results: Dict[str, Any], manufacturer: Manufacturer, session: Session) -> PriceBook:
        """Load price book."""
    ...
    def _load_finishes(self, finish_items: List[Dict[str, Any]], manufacturer: Manufacturer, session: Session) -> int:
        """Load finish symbols."""
        count = 0
        ...
        for finish_item in finish_items:
            ...
            existing_finish = session.query(Finish).filter_by(
                manufacturer_id=manufacturer.id,
    ...
    def _load_products(self, product_items: List[Dict[str, Any]], price_book: PriceBook, session: Session) -> int:
        """Load products."""
        count = 0
    ...
    def _load_options(self, option_items: List[Dict[str, Any]], price_book: PriceBook, session: Session) -> int:
    ...
    def _load_rules(self, rule_items: List[Dict[str, Any]], price_book: PriceBook, session: Session) -> int:
    ...
    def _get_or_create_family(self, product_data: Dict[str, Any], manufacturer_id: int, session: Session) -> Optional[ProductFamily]:
        """Get or create product family."""
        series = product_data.get('series') or product_data.get('model', '')
        if not series:
            return None

        family = session.query(ProductFamily).filter_by(
